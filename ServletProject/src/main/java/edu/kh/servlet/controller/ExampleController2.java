...
//Servlet 관련 코드를 작성하기 위해서 HttpServlet 클래스 상속을 받아야 함.
...
public class ExampleController2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//HttpServletRequset : 요청 데이터를 담고있는 객체
		String orderer = req.getParameter("orderer");
		String coffee = req.getParameter("coffee");
		String type = req.getParameter("type");
		/*name속성 값이 같은 파라미터가 여러개인 경우 (checkbox)
		...
		// 응답 데이터 형식, 문자 인코딩 지정
		resp.setContentType("text/html; charset=UTF-8"); // 오류주의! ( 안쓰면 한글 출력 문제) 
		
		// 응답용 스트림(서버 -> 클라이언트) 얻어오기
		PrintWriter out = resp.getWriter(); // 오류주의! 
